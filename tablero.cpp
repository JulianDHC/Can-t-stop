...
public:
    ...
    bool colocar_ficha(int suma, int jugador) 
    {
        int fila = suma - 2;
        ...
        if (ficha_en_fila) 
        {
            ...
            if (col_anterior + 1 < guia[fila]) 
            {
                ...
                return true;  // Indicar que se movió una ficha
            }
            else 
            {
        ...
        else 
        {
            ...
            if (posiciones_jugadores[jugador].size() < 3) 
            {
                ...
                return true;  // Indicar que se colocó una nueva ficha
            }
            else 
            {
                cout << "El jugador ya tiene 3 fichas activas en el tablero." << endl;
                return false;  // No se puede colocar más fichas
            }
        }
    }

    void borrarFichas(int jugador) 
    {
        // Borrar todas las fichas del jugador
        for (auto &pos : posiciones_jugadores[jugador]) 
        {
            int fila = pos.first;
            int columna = pos.second;
            ptr[fila][columna].c_[jugador] = '.';  // Borrar ficha
        }

        // Limpiar las posiciones de las fichas
        posiciones_jugadores[jugador].clear();
    }
